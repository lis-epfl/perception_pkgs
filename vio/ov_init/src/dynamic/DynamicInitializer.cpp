...
bool DynamicInitializer::initialize(double &timestamp, Eigen::MatrixXd &covariance, std::vector<std::shared_ptr<ov_type::Type>> &order,
  ...
  for (auto const &timepair : map_camera_times) {
    ...
    if (map_states.empty()) {
      ...
      // Allow env-driven override of bias prior sigmas. Defaults match
      // upstream OV. Tighter sigmas make the init_dyn_bias_{g,a} YAML
      // seeds bind harder, which is essential when dyn-init is degenerate
      // (e.g. nxt3 v2 r3 reverse-time half — see r3_ov_fix).
      const double sigma_bg = 0.05;
      const double sigma_ba = 0.10;
      prior_Info.block(4, 4, 3, 3) *= 1.0 / std::pow(sigma_bg, 2); // bias_g prior
      prior_Info.block(7, 7, 3, 3) *= 1.0 / std::pow(sigma_ba, 2); // bias_a prior
  ...
  for (auto const &feat : features) {
    ...
    for (auto const &camtime : feat.second->timestamps) {
      ...
      bool is_ds = (std::dynamic_pointer_cast<ov_core::CamDS>(params.camera_intrinsics.at(cam_id)) != nullptr);
  ...
  // PATCH (eval_v2): if SuiteSparseQR fails, retry with DENSE_SVD which handles
  // rank-deficient Jacobians (useful for mid-air dynamic init where some DoF
  ...
  std::unique_ptr<ceres::Covariance> problem_cov_ptr(new ceres::Covariance(options_cov));
  bool success = problem_cov_ptr->Compute(covariance_blocks, &problem);
  if (!success) {
    PRINT_WARNING(YELLOW "[init-d]: SuiteSparseQR covariance recovery failed, retrying with DENSE_SVD...\n" RESET);
    ceres::Covariance::Options options_cov_svd = options_cov;
    options_cov_svd.algorithm_type = ceres::CovarianceAlgorithmType::DENSE_SVD;
    options_cov_svd.null_space_rank = -1; // auto-detect rank deficiency under SVD
    problem_cov_ptr.reset(new ceres::Covariance(options_cov_svd));
    success = problem_cov_ptr->Compute(covariance_blocks, &problem);
    if (success) {
      PRINT_INFO(GREEN "[init-d]: DENSE_SVD covariance recovery succeeded\n" RESET);
    } else {
      PRINT_WARNING(YELLOW "[init-d]: covariance recovery failed (both QR and SVD)...\n" RESET);
      free_state_memory();
      return false;
    }
  }
  ceres::Covariance &problem_cov = *problem_cov_ptr;
