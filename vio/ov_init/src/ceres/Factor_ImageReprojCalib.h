...
public:
  ...
  /**
   * @brief Constructor with explicit distortion type
   * @param uv_meas_ Raw pixel uv measurement
   * @param pix_sigma_ Raw pixel uncertainty
   * @param is_fisheye_ If pinhole-equi (KB4) distortion
   * @param is_ds_ If double-sphere distortion (overrides is_fisheye_)
   */
  Factor_ImageReprojCalib(const Eigen::Vector2d &uv_meas_, double pix_sigma_, bool is_fisheye_, bool is_ds_);
