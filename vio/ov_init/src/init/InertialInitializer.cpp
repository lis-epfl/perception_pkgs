...
bool InertialInitializer::initialize(double &timestamp, Eigen::MatrixXd &covariance, std::vector<std::shared_ptr<ov_type::Type>> &order,
  ...
  if (params.init_max_disparity > 0) {
    ...
    int feat_thresh = 15;
    bool force_static = params.init_force_static;
    if (!force_static && (num_features0 < feat_thresh || num_features1 < feat_thresh)) {
      PRINT_WARNING(YELLOW "[init]: not enough feats to compute disp: %d,%d < %d\n" RESET, num_features0, num_features1, feat_thresh);
      return false;
    }
    if (force_static && (num_features0 < feat_thresh || num_features1 < feat_thresh)) {
      PRINT_INFO(YELLOW "[init]: feats below thresh (%d,%d<%d) but FORCE_STATIC continuing\n" RESET, num_features0, num_features1, feat_thresh);
      avg_disp0 = 0; avg_disp1 = 0;  // assume still
    }
  ...
  // Patch: config key init_force_static forces static init
  if (params.init_force_static) {
    is_still = true; disparity_detected_moving_1to0 = false; disparity_detected_moving_2to1 = false;
  }
