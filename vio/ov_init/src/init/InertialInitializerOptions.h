...
struct InertialInitializerOptions {
  ...
  /// Force the static initializer path: treat the platform as still (skips the
  /// disparity/feature-count stillness gates; the static initializer's own IMU
  /// variance check still applies). Use when the platform reliably starts at
  /// rest (e.g. drone on the ground with ZUPT) to anchor the trajectory in the
  /// pre-takeoff static phase. Env OV_FORCE_STATIC_INIT also enables this.
  bool init_force_static = false;
  ...
  void print_and_load_initializer(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    ...
    if (parser != nullptr) {
      ...
      parser->parse_config("init_force_static", init_force_static, false);
