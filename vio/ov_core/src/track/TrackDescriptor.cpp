...
void TrackDescriptor::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  // Stash current timestamp (ns) for the XFeat precomputed loader.
...
void TrackDescriptor::perform_detection_monocular(const cv::Mat &img0, const cv::Mat &mask0, std::vector<cv::KeyPoint> &pts0,
  ...
  // Extract our features. Default: FAST grid + ORB descriptors. XFeat mode:
  // load precomputed XFeat keypoints + 64-D float descriptors per timestamp.
...
void TrackDescriptor::robust_match(const std::vector<cv::KeyPoint> &pts0, const std::vector<cv::KeyPoint> &pts1, const cv::Mat &desc0,
  ...
  // Guard: knnMatch requires both descriptor matrices to be non-empty and
  // type-consistent. With ORB on textureless frames we sometimes see 0 rows on
  // one side which trips an OpenCV assertion. Skip cleanly.
  ...
  // Per-call deterministic seed (see TrackKLT.cpp).
  {
