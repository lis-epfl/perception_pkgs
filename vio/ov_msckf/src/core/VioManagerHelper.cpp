...
bool VioManager::try_to_initialize(const ov_core::CameraData &message) {
  ...
  std::thread thread([&] {
    ...
    if (success) {
      ...
      // PATCH (eval_v2): skip any cam timestamp that equals or precedes state->_timestamp
      // (can happen if dyn-init returns a state timestamp that matches a queued cam frame).
      ...
      for (size_t i = 0; i < camera_timestamps_to_init.size(); i += clone_rate) {
        double t_cam = camera_timestamps_to_init.at(i);
        if (t_cam <= state->_timestamp + 1e-6) {
          continue;
        }
        propagator->propagate_and_clone(state, t_cam);
