...
struct VioManagerOptions {
  ...
  /// Update-decimation: minimum seconds between EKF propagate+clone+update per
  /// lead camera. Frames arriving sooner are still TRACKED (KLT stays alive at
  /// full camera rate) but skip the filter update, so clones span a larger
  /// parallax baseline. <=0 disables (update every tracked frame, upstream
  /// behavior). Env OV_UPDATE_MIN_DT overrides when set.
  double update_min_dt = -1.0;

  /// If we should try to load a mask and use it to reject invalid features
  bool use_mask = false;

  /// Mask images for each camera (may be rebuilt at runtime if auto+dynamic)
  std::map<size_t, cv::Mat> masks;

  // === Auto fisheye-disk mask (computed at startup from intrinsics) ===
  /// Enable auto-mask: compute a circular disk mask from each cam's calibrated cx,cy + r=f*theta_max - inset
  bool mask_fisheye_auto = false;
  /// Half-FOV of the lens in radians (global default); per-cam overrides via "mask_fisheye_theta_max{i}"
  double mask_fisheye_theta_max = 1.83;
  /// Per-cam theta_max overrides (cam_id -> radians)
  std::map<int, double> mask_fisheye_theta_max_per_cam;
  /// Safety inset (px) shrunk inward from r=f*theta_max
  double mask_fisheye_inset_px = 4.0;
  /// Also reject the bottom fraction of the image (kills drone-frame / landing-gear
  /// features that move with the body). 0 = disabled. Per-cam override via
  /// "mask_bottom_frac{N}".
  double mask_bottom_frac = 0.0;
  std::map<int, double> mask_bottom_frac_per_cam;
  /// Rebuild the mask online when intrinsics drift beyond threshold
  bool mask_fisheye_dynamic = true;
  /// L1 drift in (fx,fy,cx,cy) that triggers a rebuild (px)
  double mask_fisheye_dyn_thresh_px = 1.0;
  ...
  void print_and_load_state(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      ...
      // Optional: generate a circular fisheye-disk mask at startup from the
      // calibrated cx,cy and r = (fx+fy)/2 * theta_max - inset (Kannala-Brandt
      // equidistant: r_pix = f * theta). Avoids needing a per-bag mask PNG.
      parser->parse_config("mask_fisheye_auto", mask_fisheye_auto, false);
      parser->parse_config("mask_fisheye_theta_max", mask_fisheye_theta_max, false);
      parser->parse_config("mask_fisheye_inset_px", mask_fisheye_inset_px, false);
      parser->parse_config("mask_bottom_frac", mask_bottom_frac, false);
      parser->parse_config("mask_fisheye_dynamic", mask_fisheye_dynamic, false);
      parser->parse_config("mask_fisheye_dyn_thresh_px", mask_fisheye_dyn_thresh_px, false);
      if (use_mask) {
        for (int i = 0; i < state_options.num_cameras; i++) {
          if (mask_fisheye_auto) {
            // Compute the disk geometry from the camera's calibrated intrinsics.
            auto cam = camera_intrinsics.at(i);
            int W = cam->w();
            int H = cam->h();
            Eigen::VectorXd v = cam->get_value();
            double fx = v(0), fy = v(1), cx = v(2), cy = v(3);
            double f = 0.5 * (fx + fy);
            // Allow per-cam theta_max override (e.g. "mask_fisheye_theta_max3"
            // to override just cam3). Falls back to the global default.
            double theta_max_i = mask_fisheye_theta_max;
            std::string per_cam_node = "mask_fisheye_theta_max" + std::to_string(i);
            parser->parse_config(per_cam_node, theta_max_i, false);
            mask_fisheye_theta_max_per_cam[i] = theta_max_i;
            double r_pix = f * theta_max_i - mask_fisheye_inset_px;
            if (r_pix < 1.0) r_pix = 1.0;
            // OpenVINS convention: white(255) = REJECT, black(0) = ACCEPT.
            ...
            // Optional bottom-strip mask (per-cam override allowed)
            double bottom_frac_i = mask_bottom_frac;
            std::string per_cam_bf = "mask_bottom_frac" + std::to_string(i);
            parser->parse_config(per_cam_bf, bottom_frac_i, false);
            mask_bottom_frac_per_cam[i] = bottom_frac_i;
            ...
            masks.insert({i, mask});
            PRINT_INFO("VioManager(): auto fisheye mask cam%d  f=%.2f cx,cy=(%.1f,%.1f) "
                       "theta_max=%.3f -> r=%.1f px  bottom_frac=%.3f  (image %dx%d)\n",
                       i, f, cx, cy, theta_max_i, r_pix, bottom_frac_i, W, H);
            continue;
          }
