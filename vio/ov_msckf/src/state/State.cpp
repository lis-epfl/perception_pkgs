...
State::State(StateOptions &options) {
  ...
  // Sigma priors — env-overridable. Defaults match upstream.
  // OV_PRIOR_DT_SIG  : timeoffset (sec, default 0.01)
  // OV_PRIOR_EXTR_Q_SIG: cam-imu orientation (rad, default 0.005)
  // OV_PRIOR_EXTR_T_SIG: cam-imu translation (m, default 0.015)
  // OV_PRIOR_INTR_F_SIG: cam intrinsics fx/fy/cx/cy (px, default 1.0)
  // OV_PRIOR_INTR_D_SIG: cam distortion coeffs (default 0.005)
  const double sig_dt = env_double("OV_PRIOR_DT_SIG", 0.01);
  const double sig_eq = env_double("OV_PRIOR_EXTR_Q_SIG", 0.005);
  const double sig_et = env_double("OV_PRIOR_EXTR_T_SIG", 0.015);
  const double sig_if = env_double("OV_PRIOR_INTR_F_SIG", 1.0);
  const double sig_id = env_double("OV_PRIOR_INTR_D_SIG", 0.005);

  if (_options.do_calib_camera_timeoffset) {
    _Cov(_calib_dt_CAMtoIMU->id(), _calib_dt_CAMtoIMU->id()) = std::pow(sig_dt, 2);
  }
  if (_options.do_calib_camera_pose) {
    for (int i = 0; i < _options.num_cameras; i++) {
      _Cov.block(_calib_IMUtoCAM.at(i)->id(), _calib_IMUtoCAM.at(i)->id(), 3, 3) = std::pow(sig_eq, 2) * Eigen::MatrixXd::Identity(3, 3);
      _Cov.block(_calib_IMUtoCAM.at(i)->id() + 3, _calib_IMUtoCAM.at(i)->id() + 3, 3, 3) =
          std::pow(sig_et, 2) * Eigen::MatrixXd::Identity(3, 3);
    }
  }
  if (_options.do_calib_camera_intrinsics) {
    for (int i = 0; i < _options.num_cameras; i++) {
