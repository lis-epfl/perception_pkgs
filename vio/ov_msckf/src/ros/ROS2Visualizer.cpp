...
ROS2Visualizer::ROS2Visualizer(std::shared_ptr<rclcpp::Node> node, std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim)
  ...
  if (_app->get_params().use_multi_threading_pubs) {
    std::thread thread([&] {
      // race-fix: wait for the rclcpp publisher members to be fully constructed
...
void ROS2Visualizer::setup_subscribers(std::shared_ptr<ov_core::YamlParser> parser) {
  ...
  if (_app->get_params().state_options.num_cameras == 2) {
    ...
    PRINT_INFO("subscribing to cam (stereo): %s\n", cam_topic1.c_str());
             ...
             (_app->get_params().state_options.num_cameras % 2 == 0) &&
             _app->get_params().use_stereo) {
    // Multi-stereo (>= 4 cams, even): one ApproximateTime sync per (2p, 2p+1) pair
    int ncam = _app->get_params().state_options.num_cameras;
    for (int p = 0; p < ncam / 2; p++) {
      int idL = 2 * p;
      int idR = 2 * p + 1;
      std::string cam_topicL, cam_topicR;
      _node->declare_parameter<std::string>("topic_camera" + std::to_string(idL), "/cam" + std::to_string(idL) + "/image_raw");
      _node->get_parameter("topic_camera" + std::to_string(idL), cam_topicL);
      _node->declare_parameter<std::string>("topic_camera" + std::to_string(idR), "/cam" + std::to_string(idR) + "/image_raw");
      _node->get_parameter("topic_camera" + std::to_string(idR), cam_topicR);
      parser->parse_external("relative_config_imucam", "cam" + std::to_string(idL), "rostopic", cam_topicL);
      parser->parse_external("relative_config_imucam", "cam" + std::to_string(idR), "rostopic", cam_topicR);
      auto image_subL = std::make_shared<message_filters::Subscriber<sensor_msgs::msg::Image>>(_node, cam_topicL);
      auto image_subR = std::make_shared<message_filters::Subscriber<sensor_msgs::msg::Image>>(_node, cam_topicR);
      auto sync = std::make_shared<message_filters::Synchronizer<sync_pol>>(sync_pol(10), *image_subL, *image_subR);
      sync->registerCallback(std::bind(&ROS2Visualizer::callback_stereo, this, std::placeholders::_1, std::placeholders::_2, idL, idR));
      sync_cam.push_back(sync);
      sync_subs_cam.push_back(image_subL);
      sync_subs_cam.push_back(image_subR);
      PRINT_INFO("subscribing to cam (stereo): %s\n", cam_topicL.c_str());
      PRINT_INFO("subscribing to cam (stereo): %s\n", cam_topicR.c_str());
    }
  } else {
...
void ROS2Visualizer::callback_inertial(const sensor_msgs::msg::Imu::SharedPtr msg) {
  ...
  // NOTE(deterministic-live, env OV_DET_LIVE=1): replicate the serial runner's
  // stamp-deterministic input view. IMU is buffered here and released to the
  // estimator only up to each camera frame's stamp right before that frame is
  // fed (see drain loop below). This removes the arrival-time dependence of the
  // initializer/ZUPT view of the IMU buffer (identical input stream => identical
  // output, matching the offline serial runner on the same binary).
  static const bool det_live = [] { const char *e = std::getenv("OV_DET_LIVE"); return e && *e && *e != '0'; }();
  if (det_live) {
    std::lock_guard<std::mutex> lck_imu(imu_det_queue_mtx);
    imu_det_queue.push_back(message);
    imu_arrival_watermark = std::max(imu_arrival_watermark, message.timestamp);
  } else {
  ...
  // Deterministic-live path: WATERMARK MERGE. Release the merged (IMU, camera)
  // event stream strictly in stamp order, but only up to the arrival watermark
  //   wm = min(latest-arrived IMU stamp, min over cams of latest-arrived stamp)
  // so the released prefix is a pure function of message stamps, independent of
  // arrival timing/interleave/load (per-topic delivery is in-order/reliable).
  // Ties at equal stamp: IMU before camera (serial-runner semantics), cameras
  // by cam-id (queue sort). Staleness escape: a camera lagging >1s behind the
  // IMU watermark is excluded (logged) so a dead camera cannot stall the filter.
  if (det_live) {
    std::lock_guard<std::mutex> lck(camera_queue_mtx);
    std::lock_guard<std::mutex> lck_imu(imu_det_queue_mtx);
    std::sort(imu_det_queue.begin(), imu_det_queue.end(),
              [](const ov_core::ImuData &a, const ov_core::ImuData &b) { return a.timestamp < b.timestamp; });
    int ncams = _app->get_params().state_options.num_cameras;
    while (true) {
      // recompute the watermark each iteration: arrivals continue while we drain,
      // so a single long drain invocation must be able to catch up fully
      // (stale one-shot watermark left the post-playback backlog stuck).
      double wm = imu_arrival_watermark;
      for (int c = 0; c < ncams; c++) {
        double cw = cam_arrival_watermark.count(c) ? cam_arrival_watermark.at(c) : -1e18;
        if (cw < imu_arrival_watermark - 1.0)
          continue; // stale camera excluded (dead-camera escape)
        wm = std::min(wm, cw);
      }
      bool has_imu = !imu_det_queue.empty() && imu_det_queue.front().timestamp <= wm;
      bool has_cam = !camera_queue.empty() && camera_queue.at(0).timestamp <= wm;
      if (!has_imu && !has_cam)
        break;
      bool take_imu = has_imu && (!has_cam || imu_det_queue.front().timestamp <= camera_queue.at(0).timestamp);
      if (take_imu) {
        _app->feed_measurement_imu(imu_det_queue.front());
        visualize_odometry(imu_det_queue.front().timestamp);
        imu_det_queue.pop_front();
      } else {
        auto rT0_1 = boost::posix_time::microsec_clock::local_time();
        double update_dt = 100.0 * (wm - camera_queue.at(0).timestamp);
        PRINT_INFO("[FEED] %.9f cam%d\n", camera_queue.at(0).timestamp, camera_queue.at(0).sensor_ids.at(0));
  ...
  std::thread thread([&] {
    ...
    size_t num_unique_cameras;
    if (params.state_options.num_cameras == 2) {
      num_unique_cameras = 1;
    } else if (params.state_options.num_cameras % 2 == 0 && params.use_stereo) {
      // Multi-stereo: queue has one msg per pair, so unique cams = num pairs
      num_unique_cameras = params.state_options.num_cameras / 2;
    } else {
    ...
    if (unique_cam_ids.size() == num_unique_cameras) {
      ...
      // Multi-pair-safe drain with timeout. Preferred: only drain the head when every OTHER pair has
      // a message strictly newer than the head's timestamp (guarantees in-lockstep processing and
      // prevents the `std::out_of_range: map::at` crash from a burst marginalizing the silent
      // pair's anchor clones). Bypass: if the head is older than `imu_inC - max_hold` we drain
      // anyway — otherwise a permanently lagging pair would freeze all camera updates.
      // The defensive feature-drop guards in UpdaterMSCKF/UpdaterSLAM handle any residual edge
      // cases when we drain under the bypass.
      const double max_hold = 0.5; // seconds; ~15 frames at 30Hz pair rate
      auto can_drain_head = [&](double head_ts, int head_pair_id) {
        if (num_unique_cameras <= 1) return true;
        // Bypass: too stale to keep waiting
        if (head_ts < timestamp_imu_inC - max_hold) return true;
        std::set<int> pairs_with_future;
        for (const auto &m : camera_queue) {
          int p = m.sensor_ids.at(0);
          if (p != head_pair_id && m.timestamp > head_ts) pairs_with_future.insert(p);
        }
        return pairs_with_future.size() + 1 >= num_unique_cameras;
      };
      while (!camera_queue.empty() && camera_queue.at(0).timestamp < timestamp_imu_inC &&
             can_drain_head(camera_queue.at(0).timestamp, camera_queue.at(0).sensor_ids.at(0))) {
        auto rT0_1 = boost::posix_time::microsec_clock::local_time();
...
void ROS2Visualizer::callback_monocular(const sensor_msgs::msg::Image::SharedPtr msg0, int cam_id0) {
  ...
  {
    // arrival watermark must advance on EVERY arrival (throttled frames included)
    std::lock_guard<std::mutex> lck(camera_queue_mtx);
    auto it = cam_arrival_watermark.find(cam_id0);
    if (it == cam_arrival_watermark.end() || timestamp > it->second)
      cam_arrival_watermark[cam_id0] = timestamp;
  }
  ...
  // NOTE(deterministic-live): sort with an explicit (timestamp, sensor_id) key.
  // CameraData::operator< compares timestamps only and std::sort is unstable, so
  // the relative order of the 4 same-stamp mono frames was arrival-order (racy),
  // which made the 4 sequential EKF updates trial-dependent (live nondeterminism).
  std::lock_guard<std::mutex> lck(camera_queue_mtx);
  camera_queue.push_back(message);
  std::sort(camera_queue.begin(), camera_queue.end(), [](const ov_core::CameraData &a, const ov_core::CameraData &b) {
    if (a.timestamp != b.timestamp)
      return a.timestamp < b.timestamp;
    return a.sensor_ids.at(0) < b.sensor_ids.at(0);
  });
}
...
void ROS2Visualizer::callback_stereo(const sensor_msgs::msg::Image::ConstSharedPtr msg0, const sensor_msgs::msg::Image::ConstSharedPtr msg1,
  ...
  PRINT_INFO("[stereo cb] pair (%d,%d) ts=%.4f\n", cam_id0, cam_id1,
             msg0->header.stamp.sec + msg0->header.stamp.nanosec * 1e-9);
