...
protected:
  ...
  // Deterministic-live mode (OV_DET_LIVE=1): IMU buffered here and released via a
  // stamp-ordered watermark merge (arrival-invariant input view).
  std::deque<ov_core::ImuData> imu_det_queue;
  std::mutex imu_det_queue_mtx;
  std::map<int, double> cam_arrival_watermark;  // guarded by camera_queue_mtx
  double imu_arrival_watermark = -1e18;         // guarded by imu_det_queue_mtx
