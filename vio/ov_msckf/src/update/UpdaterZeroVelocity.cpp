...
bool UpdaterZeroVelocity::try_update(std::shared_ptr<State> state, double timestamp) {
  ...
  // Image disparity (leading motion signal) — computed EARLY so it can smoothly relax the ZUPT below.
  double time0_cam = state->_timestamp, time1_cam = timestamp;
  int num_features = 0;
  ...
  if (override_with_disparity_check) {
    ...
    // Disparity stats (disp_avg, num_features) already computed above for the gradual relaxation.
