...
void UpdaterSLAM::update(std::shared_ptr<State> state, std::vector<std::shared_ptr<Feature>> &feature_vec) {
  ...
  while (it0 != feature_vec.end()) {
    ...
    // Defensive guard: drop if landmark or its anchor clone is gone (race during multi-stereo).
    auto lm_it = state->_features_SLAM.find((*it0)->featid);
    ...
    if (landmark->_anchor_clone_timestamp > 0 &&
